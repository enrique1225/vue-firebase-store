		...
		>	
			...
			<div class="dropdown-content">
				<div class="dropdown-top white--text primary">
					<span class="white--text fs-14 fw-bold d-block">{{this.current_user}}</span>
				</div>
				<v-list class="dropdown-list">
					<template v-for="userLink in userLinks">
						<template v-if="userLink.id !== 4">						
							<v-list-item :key="userLink.id">
...
export default {
	...
	created(){
		this.current_user = auth.currentUser.email ? auth.currentUser.email : "";
	},
	methods: {
		logoutUser() {
