<template>
	<div>
        ...
        <delete-confirmation-dialog
            ...
            @onConfirm="updateStore(router, lastRouter)"
        >
        </delete-confirmation-dialog>
...
import { getLastSubLayout } from "Helpers/helpers";
...
export default {
    ...
    created() {
        ...
        this.lastRouter = getLastSubLayout(this.$router);
        //... get this store detail informations
        var _that = this;
        this.getOrderDetail(this.router, this.lastRouter).then(response => {
	...
	methods:{
        ...
        async getOrderDetail(resId, orderId){
            try {

                //... get sub collections
                var orderDoc = await usersRef
                    .doc(resId)
                    .collection("orders")
                    .doc(orderId)
                    .get();
        ...
        updateStore(docId, orderId){
            var test_item = {item: this.itemsInfo}
            console.log(test_item)
            usersRef.doc(docId).collection("orders").doc(orderId).get().then(() => 
            
                usersRef.doc(docId).collection("orders").doc(orderId).update({
